...
import sys, resource, shutil, os, copy
...
def load_trace(fname):
    p = pandas.read_csv('.traces/' + fname, delimiter='/', header=None, dtype=int)
...
def main():
    ...
    print('[*] Evaluating {} input files (this may take a while)...'.format(len(FILES)))

    all_counted = Counter()
    smallest_input_for_tuple = {}
    for i, fname in enumerate(FILES):
        print('\r    Processing file {cur}/{count}... '.format(cur=i+1, count=len(FILES)),
              end='')
        ...
        for t in tuples:
            if not t in smallest_input_for_tuple or smallest_input_for_tuple[t][0] > SIZE:
    ...
    for i, tup in enumerate(RARITY_ORDER):
        if (i % 1000 == 0 or not tup in already_have):
            print(('\r    Processing tuple {curr}/{total}, '+
                   'currently have {curr_have}/{total} in {files} files... ').format(
                       curr=i+1, total=len(RARITY_ORDER), curr_have=len(already_have),
                       files=len(chosen_files)), end='')
    ...
    print("[*] Narrowed down to {count} files, copying to '{out_dir}'...".format(
        count=len(chosen_files), out_dir=OUT_DIR))
